      ...
      <table className="mb-10 border">
        <tbody>
          {Object.entries(profile).map(([k, v]) => (
            <tr key={k}>
              <th className="border px-3 py-1 text-left bg-gray-200">{k}</th>
          ...
          <tbody>
            {accounts.map(a => (
              <tr key={a.accountID}>
                ...
                <td className="border px-2">{a.closeDate ?? "—"}</td>
                <td className="border px-2">${Number(a.balance).toFixed(2)}</td>
              </tr>
            ))}
          </tbody>
        </table>
